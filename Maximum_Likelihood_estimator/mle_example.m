...
Ehat = (sum(T_data.^2))/(sum(T_data.*e_data))
sigmahat = sqrt((1/n) * sum((e_data - T_data./Ehat).^2))

% In = [sum(T_data.^2) * Ehat^(-4) * sigmahat^(-2), 0
%     0, n*sigmahat^(-2)-sum(3.*(e_data - T_data./Ehat).^2) * sigmahat^(-4)];
In = [sum(T_data.^2) * Ehat^(-4) * sigmahat^(-2), 0
     0, 2*n*sigmahat^(-2)];
